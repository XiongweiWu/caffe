...
void FocalLossLayer<Dtype>::LayerSetUp(
  ...
  //set Focal Loss parameter
  FocalLossParameter focal_loss_param = this->layer_param_.focal_loss_param();
  gamma = focal_loss_param.gamma();
  alpha = focal_loss_param.alpha();
  LOG(INFO) << "Gamma value: " << gamma;
  ...
  /// only support valid normalization
...
void FocalLossLayer<Dtype>::Forward_cpu(
  ...
  for (int i = 0; i < outer_num_; ++i) {
    ...
    for (int j = 0; j < inner_num_; j++) {
      /// classifier index, inner_num_ = 1 for example
      ...
      pt = std::max(pt, Dtype(FLT_MIN));
      loss -= alphat * pow((1-pt), gamma) * log(pt);
      /// only positive samples are used to smooth
      if (label_value!=0) {++count;}
      //++count;
    }
  }
  /// VALID normalization type here
  Dtype normalizer = LossLayer<Dtype>::GetNormalizer(
      normalization_, outer_num_, inner_num_, count);
  //Dtype normalizer = count;
      ...
      for (int j = 0; j < inner_num_; ++j) {
        ...
        for (int c = 0; c < bottom[0]->shape(softmax_axis_); ++c) {
          ...
          if (has_ignore_label_ && label_value == ignore_label_) {
           ...
           pt = std::max(pt, Dtype(FLT_MIN));
           Dtype alphat = (alpha==1) ? 1 : (label_value==1) ? alpha : (1-alpha);
           Dtype pi = prob_data[i * dim + c * inner_num_ + j];

           if (c== label_value){
               bottom_diff[i * dim + c * inner_num_ + j] = 
                   alphat* pow((1-pt), gamma) * (gamma*pt*log(pt)+pt-1);
           }
           else{
               bottom_diff[i * dim + c * inner_num_ + j] = 
                   alphat* pow(1-pt, gamma-1) * (-gamma*log(pt)*pt*pi + (1-pt)*pi);
           }
           ///++count;
          }
        }/// x1,x2
