...
__global__ void FocalLossForwardGPU(const int nthreads, 
          ...
          Dtype* counts) {
  CUDA_KERNEL_LOOP(index, nthreads) { ///nthreads: N*H
    ...
    if (has_ignore_label_ && label_value == ignore_label_) {
      ...
      Dtype alphat = (alpha==1) ? 1 : (label_value==1) ? alpha : (1-alpha);

      pt = max(pt, Dtype(FLT_MIN));
      loss[index] = -alphat*powf((1-pt), gamma) * log(pt);
      if (label_value != 0)
        counts[index] = 1;
    }
  }
...
void FocalLossLayer<Dtype>::Forward_gpu(
    ...
    //N*K*1*1, dim: K, outer_num_: N, inner_num_: 1
    ...
    FocalLossForwardGPU<Dtype><<<CAFFE_GET_BLOCKS(nthreads),
        CAFFE_CUDA_NUM_THREADS>>>(nthreads, gamma, alpha, prob_data, label, loss_data,
...
__global__ void FocalLossBackwardGPU(const int nthreads, 
	  const float gamma, const float alpha, const Dtype* prob_data,
	  const Dtype* top,
  ...
  CUDA_KERNEL_LOOP(index, nthreads) {
    ...
    for (int c = 0; c < channels; ++c) {
      if (has_ignore_label_ && label_value == ignore_label_) {
        ...
        Dtype alphat = (alpha==1) ? 1 : (label_value==1) ? alpha : (1-alpha);
	    pt = max(pt, Dtype(FLT_MIN));
        Dtype pi = prob_data[n * dim + c * spatial_dim + s];
        
        if (c == label_value){
            bottom_diff[n * dim + c * spatial_dim + s] = 
                alphat* powf((1-pt), gamma) * (gamma*pt*log(pt)+pt-1);
        }
        else {
            bottom_diff[n * dim + c * spatial_dim + s] =
                alphat* powf((1-pt), gamma-1) * (pi*(1-pt) - pi*pt*gamma*log(pt));
        }
      }
    }
    if(label_value != 0) 
...
void FocalLossLayer<Dtype>::Backward_gpu(const vector<Blob<Dtype>*>& top,
    ...
    if (propagate_down[0]) {
      ...
      FocalLossBackwardGPU<Dtype><<<CAFFE_GET_BLOCKS(nthreads),
          CAFFE_CUDA_NUM_THREADS>>>(nthreads, gamma, alpha, prob_data, top_data, label, bottom_diff,
          outer_num_, dim, inner_num_, has_ignore_label_, ignore_label_, counts);
